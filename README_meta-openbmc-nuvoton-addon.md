...
1. Add Inventory of Sensors
   ...
   * Below is config.yaml for Poleg EVB. It defines 4 events generated by 2 temperature sensors  
...
2. Add Sensor Configuration File
   ...
   * Below is a config for Poleg LM75 sensor. The sensor type is **temperature** and it name is "temp2", it has warning thresholds for **upper** and **lower** bound. The event generating option is also enabled for WARNHI and WARNLO threshold that forcing the sensor alarm to be recorded in a D-Bus object.
    ```
    LABEL_temp1=temp2
    WARNLO_temp1=28500
    WARNHI_temp1=31000
...
3. Modify D-Bus Sensor Error Metadata interface  
   * [Threshold.metadata.yaml](https://github.com/openbmc/phosphor-dbus-interfaces/blob/master/xyz/openbmc_project/Sensor/Threshold.metadata.yaml)  
     ...
     - name: CriticalLow
       ...
       inherits:
         - xyz.openbmc_project.Common.Callout.Inventory
     ```
4. Event log in WebUI  
In [Event log](https://raw.githubusercontent.com/NTC-CCBG/snapshots/e1d1733/openbmc/sensor.png) page, the event may contain a related item like below.
...
5. ipmitool sends command to get SEL records  
